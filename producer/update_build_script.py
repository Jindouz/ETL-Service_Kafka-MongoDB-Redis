...
container_exists = (
    subprocess.run("docker ps -aq --filter name=kafka-producer", capture_output=True, check=False)
...
execute_docker_command(
    "docker run -d --network=kafka-compose_kafka-network --name kafka-producer kafka-producer"
    )
