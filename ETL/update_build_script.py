...
container_exists = (
    subprocess.run("docker ps -aq --filter name=etl-service", capture_output=True, check=False)
...
execute_docker_command(
    "docker run -d --network=kafka-compose_kafka-network --name etl-service etl-service"
    )
